...
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const item = await prisma.galleryImage.findFirst({ where: { id, businessId: business.id } })
  if (!item) return NextResponse.json({ success: false, error: 'Fotoğraf bulunamadı' }, { status: 404 })
